...
class Personnage:
    """ personnage de type gÃ©nÃ©rique """
    ...
    def __init__(self, nom, espece, equipe=None):
        ...
        print("le joueur {0} a Ã©tÃ© crÃ©Ã© il est de l'Ã©quipe {1} et de l'espece {2}".format(self.nom, self.equipe, self.espece))
    ...
    def repos(self):
        ...
        if self.points_de_vie == self.points_de_vie_max and self.stamina == self.stamina_max:
            print("le joueur {0} a deja ses points de vie et sa stamina au max il n'est pas fatiguÃ©".format(self.nom))
            return
    ...
    def reveil(self):
        ...
        print(
            "le joueur {0} s'est reposÃ© pendant {1} heures, il possÃ©de maintenant {2} point de vie et {3} de stamina".format(
    ...
    def revive(self, mort):
        ...
        if self.potion_de_vie:
            if self.equipe != mort.equipe:
                print(
                    "le joueur {0} ({1}) et le joueur {2} ({3}) ne sont pas de la mÃªme Ã©quipe il faut etre de la mÃªme equipe pour revive !".format(
                        self.nom, self.equipe, mort.nom, mort.equipe))
                return
            mort.points_de_vie = self.points_de_vie_max
            mort.points_de_vie = self.stamina_max
            print(
                "le joueur {0} revient Ã  la vie avec {2} points de vie et toute sa stamina ! (il peut remercier {1}!)\nle joueur {1} ne possede plus de potion de vie".format(
    ...
    def info(self):
        if self.potion_de_vie:
            ...
            stamina = 0
        print(
            "le joueur {0} est un {1} et il possede:\n{2} points de vie \n{3} points d'attaque\n{4} de stamina\n{5} potion de vie\nil est de l'equipe{6}".format(
                self.nom, self.espece, self.points_de_vie, self.stamina, self.points_attaque, stamina, self.equipe))
    ...
    def balade(self):
        ...
        if self.espece == "mage":
            if self.potion_de_vie and random.randint(0, 1):
                ...
                print(
                    "vous vous trouvez face Ã  un loup vous vous enfuyez mais dans votre fuite vous faites tomber votre potion de vie")

        if random.randint(0, 1):
            gouter = input(
                "Oh ! on dirais bien qu'un mage qui passait par lÃ  a fait tomber une potion voulez vous la gouter ? (o/n)")
            if gouter == "o":
                if random.randint(0, 1):
...
def fin(self):
    ...
    for i in self.liste_joueur:
        if i.point_de_vie==0:
            joueur_mort+=1
        if joueur_mort==len(liste_joueur):
            return 0
    return 1